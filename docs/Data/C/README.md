...
* 指针是一种变量,其值为另一个变量的地址，即，内存位置的直接地址。必须在使用指针存储其他变量地址之前，对其进行声明。
* **指针也是一个类型，由其他类型派生**
  * 比如指向 int的指针类型
  * char 的指针类型
  * double 的指针类型
...
* **\* 是解引用运算符 在指针变量前面加 就表示这个指针指向的内存单元中的值**

* 指针变量保存的值，也就是地址 是可以变的。
* 一个变量保存另一个指针的地址，指针的指针
* 把一个指针赋值NULL，就表明是空指针。
...
* 函数传参（**数组变量是一个特殊的指针，数组变量本身表达的就是地址。**）

* 数组元素表达的是变量 需要用 & 取地址符

* 数组变量是 const指针，是常量 不能被赋值
...
const在 * 的前面 那么它前面指的东西不能被修改。

const 在 * 的后面 表示指针不能被修改。
...
函数指针是指向函数的指针变量，我们说的指针变量是指向一个整型、字符型或数组等变量。而函数指针是指向函数。 
